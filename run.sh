...
# Export REDIS_URL for the Valkey instance using the URI and credentials
if [[ -n "$VALKEY_USERNAME_REOPS_UMAMI_BETA" && -n "$VALKEY_PASSWORD_REOPS_UMAMI_BETA" ]]; then
 # export REDIS_URL="$(echo $VALKEY_URI_REOPS_UMAMI_BETA | sed "s#://#://$VALKEY_USERNAME_REOPS_UMAMI_BETA:$VALKEY_PASSWORD_REOPS_UMAMI_BETA@#")"
else
 # export REDIS_URL="$VALKEY_URI_REOPS_UMAMI_BETA"
fi
